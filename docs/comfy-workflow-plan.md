...
1. **ComfyUI Workflow session + readiness gate.** A new session type.
   User uploads or picks a workflow, sees a node-readiness panel, and
   walks every unknown node through a staged import wizard until all
   are *ready*. Phase 1 ends there — no slot mapping, no generation.
   The catalog (`comfy_packs`, `comfy_nodes`) grows as a side effect of
   import. The library gains a Comfy Nodes section as a secondary view
   of what has been imported.

2. **Workflow slot mapping.** Once a workflow's nodes are all ready,
   propose a mapping from logical slots (positive prompt, negative
   prompt, seed, width / height, main image, LoRA stack, sampler params)
   onto specific node ids using the catalog's role hints. User confirms
   or edits.

3. **Generation execution.** `comfy_client` service, the actual queue +
   websocket cycle, image fetching, persistence, progress streaming over
   SSE.
...
### Per-node import wizard — staged and retryable
...
succeed.** A failed stage shows an error and a Retry button for that
stage; the user cannot save a partial node.

**Stage 1 — Locate pack.** Walk `<comfyui_path>/custom_nodes/` looking
for an `__init__.py` whose `NODE_CLASS_MAPPINGS` mentions this
class_type (static parse, no execution of pack code). When found, read
the matching pack's `pyproject.toml`. If no pack matches but the
class_type is in `/api/object_info`, treat as built-in and bind to the
synthetic pack `ComfyUI`. Outputs: pack name, repo URL, publisher,
display name, version, raw README markdown.
...
doesn't invent input names, strict JSON-schema response, no need for
the verbosity tuning of `generate`. Defaults live in `app_settings`
under a new `default_comfy_import_settings` JSON column; per-session
override is not needed because import is global (per-`class_type`),
not per-session. Validation lives alongside the existing action
bundles in `action_settings.py`.
...
### Library — Comfy Nodes section (secondary surface)
...
### Settings — dedicated ComfyUI section
...
New endpoint groups, all under `/api/`:
...
**Workflows:**
- `POST /api/comfy/workflows` — upload a workflow JSON. Returns the
  saved row, or a 409 with `{conflict: "graph_hash", existing:
  {id, name}}` when a duplicate hash is detected; client then
  re-submits with `?on_conflict=replace|rename`.
- `GET /api/comfy/workflows` — list (id, name, created_at).
- `GET /api/comfy/workflows/{id}` — full workflow (graph_json
  included).
- `DELETE /api/comfy/workflows/{id}`.

**Readiness (per session):**
- `GET /api/comfy/sessions/{id}/readiness` — for the session's bound
  workflow, computes one entry per distinct class_type with status
  (`ready` | `needs_config` | `not_installed`) and the cached
  `comfy_nodes` row when present. Recomputed on each call.
- `POST /api/comfy/sessions/{id}/readiness/refresh` — re-queries
  `/api/object_info` from ComfyUI; used by the Refresh button on
  `not_installed` cards.

**Per-node import wizard:**
- `POST /api/comfy/nodes/{class_type}/import` — kicks off a four-stage
  import job, returns a `job_id`.
- `GET /api/comfy/nodes/{class_type}/import/{job_id}/stream` — SSE
  stream of stage events: `stage_started` / `stage_succeeded` /
  `stage_failed` / `done`. Each `stage_succeeded` carries that
  stage's outputs so the UI can show "found pack X", "got 7 inputs",
  etc. live.
- `POST /api/comfy/nodes/{class_type}/import/{job_id}/retry` — retry
  the failed stage; previously-completed stages keep their cached
  output, the wizard does not redo them.
- `DELETE /api/comfy/nodes/{class_type}/import/{job_id}` — cancel.

**Catalog (library and node detail):**
- `GET /api/comfy/nodes` — list with search/filter (matches Library's
  flat search); supports `?q=`, `?pack=`, `?builtin=`,
  `?has_description=`.
- `GET /api/comfy/nodes/{class_type}` — full node row including raw
  schema and semantic form.
- `PUT /api/comfy/nodes/{class_type}` — update editable fields
  (`description_md`, `inputs_semantic_json`, `category`); writes go
  through `comfy_node_overrides`.
...
## Phase 2 — Workflow slot mapping (sketch)

Once Phase 1 lands, the catalog supplies `role_hint` annotations for
every input of every node in any imported workflow. Phase 2:
...
## Phase 3 — Generation execution (sketch)
...
Resolved since the previous revision of this plan:
- **When to do LLM enrichment** — at import time, synchronously, per
  node, with retryable stages. Bulk batch and on-demand-only
...
- Unit tests for the static `__init__.py` parser against fixtures
  covering representative shapes: rgthree-comfy (many `from
  .py.foo import Foo` + `Foo.NAME` constant access), z-image-turbo
  (literal-string mapping, two nodes), comfyui-impact-pack (large
  pack), comfyui_essentials (multi-file split), ComfyUI-Show-Text
  (single-class minimum), and packs with missing `pyproject.toml`.
...
When Phase 1 ships, update `docs/spec/technical_specifications.md`:
...
- Add the new `comfy_import` action (alongside `analyze`, `chat`,
  `summarize`, `generate`) and its `default_comfy_import_settings`
  app-level defaults.
...
This document remains for ongoing planning of Phases 2 and 3 and the
