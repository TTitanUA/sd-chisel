...
INSERT OR IGNORE INTO families(id, display_name, prompt_guide, created_at, updated_at) VALUES
  ...
  ('flux', 'Flux',
   ...
   strftime('%s','now'), strftime('%s','now')),
  ('sd15', 'SD 1.5',
   'Stable Diffusion 1.5. Short, comma-separated tags work best. CFG 7' || char(8211) || '12. Negative prompt is important.',
   strftime('%s','now'), strftime('%s','now')),
  ('sd21', 'SD 2.1',
   'Stable Diffusion 2.1. Similar to 1.5 but larger token space. Needs explicit quality tags.',
   strftime('%s','now'), strftime('%s','now')),
  ('cascade', 'Stable Cascade',
   'Stable Cascade. Natural language. Two-stage process' || char(59) || ' prompts at both stages merged. Highly coherent.',
   strftime('%s','now'), strftime('%s','now')),
  ('hunyuan', 'HunyuanDiT',
  ...
  ('kolors', 'Kolors',
   ...
   strftime('%s','now'), strftime('%s','now')),
  ('auraflow', 'AuraFlow',
   'AuraFlow v0.3. Natural language, flow-based diffusion. Avoid overly long prompts' || char(59) || ' concise descriptions work best.',
   strftime('%s','now'), strftime('%s','now'));
