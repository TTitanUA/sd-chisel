...
function SlotSummary({ slot }: { slot: AgentInputSlot }) {
  ...
  switch (slot.kind) {
    ...
    case "prompt_guide": {
      ...
      text = cfg.mode === "auto" ? "auto-pick" : (cfg.guide_id ?? "(no guide)");
      break;
    }
    case "system": {
...
function SourceForm({
  ...
  const { session, sourceSlots } = useComfyMock();
    ...
    <>
      <div className={styles.field}>
        ...
        <div className={styles.modeRow}>
          <label className={styles.radio}>
            ...
            />
            Auto — use the model + generation type's default
          </label>
          <label className={styles.radio}>
            <input
              type="radio"
      ...
      <label className={styles.field}>
        ...
        <select
          ...
          onChange={(e) =>
            patch({ generation_type: e.currentTarget.value || null })
          }
        >
          <option value="">(infer from workflow)</option>
          {MOCK_GENERATION_TYPES.map((g) => (
      ...
      {cfg.mode === "manual" && (
        <label className={styles.field}>
          ...
          >
            ...
            {MOCK_PROMPT_GUIDES.map((g) => (
              <option key={g.id} value={g.id}>
                {g.label} — {g.families.join(", ")}
              </option>
            ))}
          </select>
        </label>
