/** Variant D — IDE-like. Left vertical tab-bar with one button per
 *  panel (Agents / Inputs / Sources / Nodes / Chat); centre toggles
 *  between agent editor and the slot-mapping tree (drawer-style);
 *  bottom panel = gallery (collapsible, like a VSCode terminal).
 *
 *  The previous Slots tab was removed once the tree drawer's
 *  built-in MAPPED SLOTS column took over the same job — see the
 *  `MappingTreeDrawer` shell. See docs/comfy-agents-ui-mock-plan.md. */
...
export function VariantDLayout({ knobsOpen }: { knobsOpen: boolean }) {
  const { session, agents, selectedAgentId, slotMap } = useComfyMock();
