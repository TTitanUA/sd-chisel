...
/** Two readonly textareas with the input's *default* (the workflow's
 *  saved value, before any agent runs) and *current* (the value the
 *  workflow will actually receive at Generate time — agent
 *  ``last_value`` for `llm` slots, ``metadata.value`` for `frozen`,
 *  resolved image filename for `user_image`). When the live value
 *  is empty the textarea shows a "no value yet — run agent"
 *  placeholder so the empty box doesn't look broken. */
function ValueSummary({ input }: { input: MappingInputRow }) {
  const { agents, sourceSlots, session } = useComfy();
  const live = resolveLiveValue(input, { agents, sourceSlots, session });
  const liveText = formatValueFull(live.value);
  const livePlaceholder =
    live.source === "llm"
      ? "no value yet — run agent"
      : live.source === "user_image"
        ? "no image bound"
        : "no value";
  return (
    <div className={styles.valueGrid}>
      <label className={styles.valueField}>
        ...
        <textarea
          ...
          value={formatValueFull(input.rawValue)}
          readOnly
          rows={3}
          spellCheck={false}
...
function EditMapped({
  ...
  const { sourceSlots, session, agents } = useComfy();
