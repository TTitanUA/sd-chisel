/** Full-viewport overlay that opens the moment Single Run starts and
 *  stays mounted until the user dismisses it. Two regions:
 *
 *  - **Pipeline strip** (top) — one chip per stage with pending /
 *    running / success / failed / skipped state and a stamp of the
 *    most recent ts.
 *  - **Event log** (bottom) — append-only, newest at the bottom; one
 *    line per server event with its stage / event / detail summary.
 *
 *  While `inProgress`, the whole workspace under the modal is locked
 *  (handled by ComfyWorkspaceLayout); the only live control is
 *  `Close` which is enabled once the run terminates. */
...
export function RunViewer({
  ...
  // Esc closes the viewer once the run is done; while running, ignore.
  useEffect(() => {
        ...
        <header className={styles.header}>
          ...
          <button
            ...
            title={runState.inProgress
              ...
              : "Close"}
          >
            {runState.inProgress ? "Running…" : "Close"}
          </button>
        </header>
